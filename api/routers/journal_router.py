...
async def get_entry(request: Request, entry_id: str, entry_service: EntryService = Depends(get_entry_service)):
    try:
        ...
        if not entity:
            raise HTTPException(
                status_code=404, detail="The entry you requested, Not found")
        ...
        raise HTTPException(
            status_code=501, detail=f"Error retrieving enrtry - {str(e)}")
...
async def delete_entry(request: Request, entry_id: str, entry_service: EntryService = Depends(get_entry_service)):
    try:
        ...
        if entry:
            ...
            return {"entry": entry, "message": "Entry deleted Successfully"}
